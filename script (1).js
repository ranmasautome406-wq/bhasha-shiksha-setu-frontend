...
// Ignore old/invalid values saved in browser storage.
const SAVED_API_URL = (localStorage.getItem("bss_api_url") || "").trim();
const API_BASE_URL =
  (SAVED_API_URL &&
   !SAVED_API_URL.includes("YOUR-BACKEND-URL") &&
   !SAVED_API_URL.includes("localhost") &&
   /^https?:\/\//i.test(SAVED_API_URL))
    ? SAVED_API_URL.replace(/\/$/, "")
    : DEPLOYED_BACKEND_URL;

// Keep the correct backend URL for future page loads.
